...
{    
    ...
    NL = max(dot(frag_n, -point_dir), 0.0);
    diffuse = light.points[i].color * light.points[i].intensity * dist_attenuation * NL;
    
    reflect_dir = reflect(point_dir, frag_n);  
    VRn = pow(max(dot(view_n, reflect_dir), 0.0), shininess);
    
    Rf = Rf0 + (1 - Rf0) * pow(1 - NL, 5); 
    ...
    NL = max(dot(frag_n, -spot_dir), 0.0);
    
    diffuse = light.spots[i].color * light.spots[i].intensity * dist_attenuation * c_attenuation * NL;
    
    reflect_dir = reflect(spot_dir, frag_n);  
    VRn = pow(max(dot(view_n, reflect_dir), 0.0), shininess);
